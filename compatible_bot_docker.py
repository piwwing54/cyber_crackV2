#!/usr/bin/env python3
"""
CYBER CRACK PRO BOT - DOCKER-COMPATIBLE VERSION WITH FILE HANDLING
Menggunakan aiogram 2.x untuk kompatibilitas dengan sistem yang ada
"""
...
@dp.message_handler(lambda message: message.text == "📁 Upload APK")
...
@dp.message_handler(lambda message: message.text == "🔍 Analyze Now")
async def analyze_now(message: types.Message):
    """Analyze command"""
...
@dp.message_handler(lambda message: message.text in ["📊 Status", "ℹ️ Help", "📋 About", "🔍 Analyze", "🔧 Crack", "🎮 Game", "💎 Premium"])
async def command_alias(message: types.Message):
...
async def handle_apk_upload(message: types.Document):
    """Handle APK file uploads with progress feedback"""
    document = message.document
    
    # Check if it's an APK, APKS, or XAPK file
